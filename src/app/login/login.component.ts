...
export class LoginComponent implements OnInit {
  ...
  loginUser(loginForm: any) {
    ...
    if (this.isForgotPassword) {
      ...
      if (
        !this.passwordFormValue?.fullname ||
        !this.passwordFormValue?.email ||
        !this.passwordFormValue?.password
      ) {
        return;
      } else if (
    ...
    if (this.isForgotPassword) {
      this.genericService
        .generateOTP({ email: this.passwordFormValue?.email })
        .subscribe((data) => {
          console.log(data);
          Swal.fire({
      ...
      this.genericService.loginUser(this.loginFormData).subscribe(
        (data: any) => {
          console.log(data);
